...
function rollupForMonths(a, b) {
    ...
    for (var f in a.data) {
        var g = f.split(":");
        if (g.length == 1) var h = g[0];
        else var i = g[0],
            h = g[1]; if (b && !b[i - 1]) continue;
...
function drawBigWindrose(a, b, c) {
    var d = 400,
        ...
        g = 20,
        h = 34,
        i = d3.select(b).append("svg:svg").attr("width", d + "px").attr("height", e + 30 + "px");
    if (b == "#windrose") var j = d3.range(.025, .151, .025),
        k = d3.range(.05, .101, .05),
        l = probabilityToRadiusScale,
        m = function (a) {
            return "" + (a * 100).toFixed(0) + "%"
        };
    else var j = d3.range(5, 20.1, 5),
        k = d3.range(5, 15.1, 5),
        l = speedToRadiusScale,
        m = function (a) {
            return "" + a + "kts"
        };
...
function plotSmallRose(a, b) {
    var c = [],
        d = selectedMonthControl.selected();
    for (var e = 1; e < 13; e++) {
        var f = 0;
        sum = 0;
...
function installGeoTiler() {
    if (d3.geo.tiler) console.log("d3.geo.tiler defined: not using ours. Good luck!");
    else {
        d3.geo.tiler = function () {
            ...
            var b = {},
                c = [],
                d = d3.geo.mercator().scale(1).translate([.5, .5]),
                e = Object,
                f = 8,
                g = null;
            return b.location = function (a) {
                ...
                function l(a) {
                    if (a.length)
                        for (var b = -1, c = a.length; ++b < c;) j.push(a[b]);
                    else
                        for (var b = -1; ++b < 4;) a[b] && l(a[b])
                }
                var j = [];
...
(function () {
    ...
    function ct(a) {
        function i(f) {
            ...
            var i = bV(this, f, b, d),
                j = bV(this, f, b === c ? cu(i) : c, d === e ? cv(i) : e);
    ...
    function dq(a, b, c) {
        function i(a, b) {
            var c = a.__domain || (a.__domain = a.domain()),
                d = a.range().map(function (a) {
                    return (a - b) / h
                });
            a.domain(c).domain(d.map(a.invert))
        }
        var d = Math.pow(2, (dc[2] = a) - c[2]),
            e = dc[0] = b[0] - d * c[0],
            f = dc[1] = b[1] - d * c[1],
            g = d3.event,
            h = Math.pow(2, a);
    ...
    d3.functor = function (a) {
        ...
        for (var a = -1, b = d3.min(arguments, f), c = new Array(b); ++a < b;)
            for (var d = -1, e, g = c[a] = new Array(e); ++d < e;) g[d] = arguments[d][a];
        return c
    ...
    d3.ease = function (a) {
        ...
        var c, d, e, f = 0,
            ...
            j, k;
        D.lastIndex = 0;
        for (d = 0; c = D.exec(b); ++d) c.index && h.push(b.substring(f, g = c.index)), i.push({
            i: h.length,
            x: c[0]
    ...
    d3.selection = function () {
        ...
        for (var e = -1, f = this.length; ++e < f;)
            for (var g = this[e], h = -1, i = g.length; ++h < i;)
                if (d = g[h]) b.push(c = a.call(d, d.__data__, h)), c.parentNode = d;
        return T(b)
    }, W.attr = function (a, b) {
        function d() {
        ...
        for (var e = -1, f = this.length; ++e < f;)
            for (var g = this[e], h = -1, i = g.length; ++h < i;)
                if (d = g[h]) {
                    ...
                    for (var j = -1, k = c.length; ++j < k;) c[j] = {
                        node: c[j],
                        delay: d.delay,
                        duration: d.duration
                    }
    ...
    var bI = ["#1f77b4", "#ff7f0e", "#2ca02c", "#d62728", "#9467bd", "#8c564b", "#e377c2", "#7f7f7f", "#bcbd22", "#17becf"],
        bJ = ["#1f77b4", "#aec7e8", "#ff7f0e", "#ffbb78", "#2ca02c", "#98df8a", "#d62728", "#ff9896", "#9467bd", "#c5b0d5", "#8c564b", "#c49c94", "#e377c2", "#f7b6d2", "#7f7f7f", "#c7c7c7", "#bcbd22", "#dbdb8d", "#17becf", "#9edae5"],
        bK = ["#393b79", "#5254a3", "#6b6ecf", "#9c9ede", "#637939", "#8ca252", "#b5cf6b", "#cedb9c", "#8c6d31", "#bd9e39", "#e7ba52", "#e7cb94", "#843c39", "#ad494a", "#d6616b", "#e7969c", "#7b4173", "#a55194", "#ce6dbd", "#de9ed6"],
        bL = ["#3182bd", "#6baed6", "#9ecae1", "#c6dbef", "#e6550d", "#fd8d3c", "#fdae6b", "#fdd0a2", "#31a354", "#74c476", "#a1d99b", "#c7e9c0", "#756bb1", "#9e9ac8", "#bcbddc", "#dadaeb", "#636363", "#969696", "#bdbdbd", "#d9d9d9"];
    d3.scale.quantile = function () {
        ...
        function e() {
            var e = a.apply(this, arguments),
                ...
                g = c.apply(this, arguments) + bO,
                h = d.apply(this, arguments) + bO,
        ...
        return e.innerRadius = function (b) {
            ...
            var e = (a.apply(this, arguments) + b.apply(this, arguments)) / 2,
                f = (c.apply(this, arguments) + d.apply(this, arguments)) / 2 + bO;
        ...
        function j(j) {
            j.each(function (k, l, m) {
                ...
                var n = d3.select(this),
                    o = a.ticks.apply(a, g),
                    p = h == null ? a.tickFormat.apply(a, g) : h,
                    q = cM(a, o, i),
                    r = n.selectAll(".minor").data(q, String),
                    s = r.enter().insert("svg:line", "g").attr("class", "tick minor").style("opacity", 1e-6),
                    t = F(r.exit()).style("opacity", 1e-6).remove(),
                    u = F(r).style("opacity", 1),
                    v = n.selectAll("g").data(o, String),
                    w = v.enter().insert("svg:g", "path").style("opacity", 1e-6),
                    x = F(v.exit()).style("opacity", 1e-6).remove(),
                    y = F(v).style("opacity", 1),
                    z, A = bq(a.range()),
                    B = n.selectAll(".domain").data([0]),
                    C = B.enter().append("svg:path").attr("class", "domain"),
                    D = F(B),
                    E = this.__chart__ || a;
...
function () {
    ...
    d3.geo = {}, d3.geo.azimuthal = function () {
        function j(c) {
            var g = c[0] * a - f,
                ...
                m = Math.cos(j),
                n = Math.sin(j),
                o = b === "stereographic" ? 1 / (1 + i * n + h * m * k) : 1,
                p = o * m * l,
                q = o * (i * m * k - h * n);
        ...
        return j.invert = function (c) {
            var g = (c[0] - e[0]) / d,
                j = (c[1] - e[1]) / d,
                k = Math.sqrt(g * g + j * j),
                l = b === "stereographic" ? 2 * Math.atan(k) : Math.asin(k),
                m = Math.sin(l),
                n = Math.cos(l);
        ...
        function j(b) {
            var c = g * (a * b[0] - f),
                j = Math.sqrt(h - 2 * g * Math.sin(a * b[1])) / g;
    ...
    d3.geo.mercator = function () {
        ...
        function l(a) {
            var b = d3.geom.polygon(a[0].map(f)),
                c = b.centroid(1),
                d = c[0],
                e = c[1],
                g = Math.abs(b.area()),
    ...
    d3.geo.greatCircle = function () {
        function f(e, f) {
            var g = b.call(this, e, f),
                ...
                s = Math.cos(l),
                t = Math.sin(l),
                e = Math.acos(p * t + o * s * Math.cos(k - i)),
                u = Math.sin(e),
                v = e / (d - 1),
                w = -v,
                x = [],
        ...
        return f.source = function (a) {
            ...
            var g = b.call(this, d, f),
                ...
                m = Math.sin((l - j) / 2),
                n = Math.sin((k - i) / 2),
                o = m * m + Math.cos(j) * Math.cos(l) * n * n;
            return e * 2 * Math.atan2(Math.sqrt(o), Math.sqrt(1 - o))
        }, f
    }
}();
if (!org) var org = {};
org.polymaps || (org.polymaps = {}),
    function (a) {
        function d(b) {
            var c = b.indexOf(":");
            return c < 0 ? b : {
        ...
        function j(a, b, c, d, e) {
            var f = Math.max(c, Math.floor(b.y0)),
                g = Math.min(d, Math.ceil(b.y1));
            ...
            var i = a.dx / a.dy,
                ...
                l = b.dx < 0;
            for (var m = f; m < g; m++) {
                var n = i * Math.max(0, Math.min(a.dy, m + k - a.y0)) + a.x0,
                    o = j * Math.max(0, Math.min(b.dy, m + l - b.y0)) + b.x0;
        ...
        a.ns = {
            ...
            return arguments.length || (a = 1, c = 0, e = 0, b = 0, d = 1, f = 0), g.zoomFraction = function (e) {
                ...
                var i = Math.pow(2, -g.zoom),
                    j = Math.pow(2, g.zoom - h);
                return {
            ...
            function j(a) {
                ...
                else if (g = a.prev) g.next = null;
                if (a.prev) a.prev.next = a.next;
                else if (f = a.next) f.prev = null
            }
            ...
            return c.peek = function (a) {
                ...
                var h = [b.zoom, b.column, b.row].join("/"),
                    j = e[h];
            ...
            function e(e) {
                var f = e.zoom < 0 ? 1 : 1 << e.zoom,
                    g = e.column;
                if (d) g = e.column % f, g < 0 && (g += f);
                else if (g < 0 || g >= f) return null;
            ...
            function B() {
                if (!z) return;
                var a = 45 / Math.pow(2, o + p - 3),
                    b = Math.max(Math.abs(u * l.x + t * l.y), Math.abs(w * l.x + v * l.y)),
                    c = f(x - b * a / m.y),
                    d = f(y + b * a / m.y);
                n.lat = Math.max(c, Math.min(d, n.lat));
                var e = Math.max(Math.abs(u * l.y + t * l.x), Math.abs(w * l.y + v * l.x)),
                    g = z[0].lon - e * a / m.x,
                    h = z[1].lon + e * a / m.x;
            ...
            d.locationCoordinate = function (b) {
                var c = a.map.locationCoordinate(b),
                    d = Math.pow(2, o);
                return c.column *= d, c.row *= d, c.zoom += o, c
            }, d.coordinateLocation = a.map.coordinateLocation, d.coordinatePoint = function (a, b) {
                var c = Math.pow(2, o - b.zoom),
                    d = Math.pow(2, o - a.zoom),
                    e = (b.column * c - a.column * d) * m.x * q,
                    f = (b.row * c - a.row * d) * m.y * q;
                return {
                    x: l.x + t * e - u * f,
                    y: l.y + u * e + t * f
                }
            }, d.pointCoordinate = function (a, b) {
                var c = Math.pow(2, o - a.zoom),
                    d = (b.x - l.x) / q,
                    e = (b.y - l.y) / q;
                return {
                ...
                var b = Math.pow(2, o + p - 3) / 45,
                    c = (a.lon - n.lon) * b * m.x,
                    d = (g(n.lat) - g(a.lat)) * b * m.y;
                return {
                    x: l.x + t * c - u * d,
                    y: l.y + u * c + t * d
                }
            }, d.pointLocation = function (a) {
                var b = 45 / Math.pow(2, o + p - 3),
                    c = (a.x - l.x) * b,
                    d = (a.y - l.y) * b;
                return {
            ...
            return C.setAttribute("visibility", "hidden"), C.setAttribute("pointer-events", "all"), d.container = function (a) {
                ...
                if (!arguments.length) return [d.pointLocation({
                    x: 0,
                    y: k.y
                }), d.pointLocation({
                    x: k.x,
                ...
                var b = d.locationPoint(a[0]),
                    c = d.locationPoint(a[1]),
                    e = Math.max((c.x - b.x) / k.x, (b.y - c.y) / k.y),
                    f = d.pointLocation({
                        x: (b.x + c.x) / 2,
                        y: (b.y + c.y) / 2
                    });
        ...
        a.layer = function (b, d) {
            ...
            function t() {
                function D(b) {
                    ...
                    return e < 0 && (e += d), {
                        locationPoint: function (b) {
                            var d = a.map.locationCoordinate(b),
                                g = Math.pow(2, c - d.zoom);
                            return {
                                x: t.x * (g * d.column - e),
                                y: t.y * (g * d.row - f)
                            }
                ...
                var b = e.map(),
                    ...
                    q = b.angle(),
                    t = b.tileSize(),
                    u = b.locationCoordinate(b.center());
                if (o != d) {
                    o < d ? r(d) : o > d ? s(d) : o = d;
                    for (var v = -4; v <= 2; v++) {
                        var w = p[v];
                ...
                var x = b.pointCoordinate(u, c),
                    y = b.pointCoordinate(u, {
                        x: l.x,
                        y: 0
                    }),
                    z = b.pointCoordinate(u, l),
                    A = b.pointCoordinate(u, {
                        x: 0,
                        y: l.y
                    });
                ...
                if (h && B > -5 && B < 3) {
                    ...
                    if (g) k(x, y, z, 0, H, K), k(z, A, x, 0, H, K);
                    else {
                        var I = Math.floor((x.column + z.column) / 2),
                            J = Math.max(0, Math.min(H - 1, Math.floor((y.row + A.row) / 2))),
                            v = Math.min(4, x.zoom);
                ...
                for (var G in F) {
                    var L = F[G],
                        C = Math.pow(2, L.level = L.zoom - u.zoom);
            ...
            var e = {},
                f = e.cache = a.cache(b, d).size(512),
                g = !0,
                h = !0,
                i, j, l, m = a.svg("g"),
                n, o, p = {};
            ...
            function d(d) {
                ...
                } else d.ready = !0, c != null && e.setAttributeNS(a.ns.xlink, "href", c), b.dispatch({
                    type: "load",
                    tile: d
                })
            }
            ...
            var c = a.layer(t, u),
                ...
                g = "org.polymaps." + a.id(),
                h = "url(#" + g + ")",
                i = d.insertBefore(a.svg("clipPath"), d.firstChild),
                j = i.appendChild(a.svg("rect")),
                k = "auto",
                l = null,
                m;
            ...
            var a = {},
                ...
                d = !0,
                e = "mouse",
                f, g, h, j = document.createElement("div"),
            ...
            function k(b) {
                ...
                case 2:
                    var d = b.touches[0],
                        ...
                        m = a.map.locationCoordinate(i[d.identifier]),
                        n = a.map.locationCoordinate(i[f.identifier]),
                        o = {
                            row: (m.row + n.row) / 2,
            ...
            function G(b) {
                var c = Math.SQRT1_2 * e,
                    d = e * .7,
                    f = e * .2,
                    g = a.svg("g"),
                    h = g.appendChild(a.svg("path")),
                    i = g.appendChild(a.svg("path"));
            ...
            function H(b) {
                var c = e * .4,
                    d = c / 2,
                    f = a.svg("g"),
                    g = f.appendChild(a.svg("path")),
                    h = f.appendChild(a.svg("path")),
                    i = f.appendChild(a.svg("path")),
                    j = f.appendChild(a.svg("path"));
            ...
            function f(b) {
                var f, g = e.firstChild,
                    h = d.size(),
                    i = d.pointLocation(c),
                    j = d.pointLocation(h),
                    k = Math.pow(2, 4 - Math.round(d.zoom()));
                i.lat = Math.floor(i.lat / k) * k, i.lon = Math.ceil(i.lon / k) * k;
                for (var l;
                    (l = d.locationPoint(i).x) <= h.x; i.lon += k) g || (g = e.appendChild(a.svg("line"))), g.setAttribute("x1", l), g.setAttribute("x2", l), g.setAttribute("y1", 0), g.setAttribute("y2", h.y), g = g.nextSibling;
                for (var m;
                    (m = d.locationPoint(i).y) <= h.y; i.lat -= k) g || (g = e.appendChild(a.svg("line"))), g.setAttribute("y1", m), g.setAttribute("y2", m), g.setAttribute("x1", 0), g.setAttribute("x2", h.x), g = g.nextSibling;
                while (g) {
                    var n = g.nextSibling;
                    e.removeChild(g), g = n
                }
            ...
            return e.setAttribute("class", "grid"), b.map = function (a) {
                ...
                if (d = a) d.on("move", f).on("resize", f), d.container().appendChild(e), d.dispatch({
                    type: "move"
                });
                return b
            }, b
...
var arc = function (a) {
    ...
    speedToColorScale = d3.scale.linear().domain([5, 25]).range(["hsl(220, 70%, 90%)", "hsl(220, 70%, 30%)"]).interpolate(d3.interpolateHsl),
    probabilityToColorScale = d3.scale.linear().domain([0, .2]).range(["hsl(0, 70%, 99%)", "hsl(0, 70%, 40%)"]).interpolate(d3.interpolateHsl),
    visWidth = 200,
    probabilityToRadiusScale = d3.scale.linear().domain([0, .15]).range([34, visWidth - 20]).clamp(!0),
    ...
    smallArcScale = d3.scale.linear().domain([0, .15]).range([5, 30]).clamp(!0),
    smallArcOptions = {
        width: 15,
        from: 5,
        to: function (a) {
    ...
    stationDb = null,
    map = null,
    po = typeof org != "undefined" ? org.polymaps : null,
    selectedMonthControl = null